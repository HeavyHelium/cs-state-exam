...
- ```<израз>``` е израз от допустим тип (типовете `bool`, `int` и `char` са допустими, реалните типове `double` и `float` - не)
...
Операторът връща присвоената стойност. Той е дясноасоциативен - напр. a = b = c = 42 $\iff$ a = (b = (c = 42)).
...
* глобални променливи - дефинирани са извън тяло на функция и са видими(освен ако не са скрити по правилото локалното скрива глобалното) из цялата програма. 
...
* Понякога искаме промените във формалните параметри да се отразяват и във фактическите параметри 
...
Символен низ наричаме последователност от символи. В C++ най-базовото им представяне е това като масиви от символи, които завършват с терминиращия символ '\0'(завършваща нула).  
...
```
* `std::strcpy(source, destination)` - копира низа `source` в масива `destination`
```cpp
// примерна реализация
