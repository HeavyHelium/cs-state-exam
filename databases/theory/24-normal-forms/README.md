...
```def``` Казваме, че декомпозицията на $R$ на релациите $S$ и $T$ без загуба тогава и само тогава, когато $S \bowtie T = R$.
...
```def```Релацията $R$ със схема $R(A)$ е в 3NF $\iff$ за всяка нетривиална ФЗ $X \to Y$ е в сила, че X е суперключ.

напр., всяка релация с 2 атрибута е в BCNF, което е следствие от вида на възможните ФЗ в такава релация 

---

### Многозначни зависимости

Казваме, че X многозначно определя $Y$ и пишем $X \rightarrow\rightarrow Y$ $\iff$ ако 2 кортежа в една релация съвпадат по всички атрибути на $X$,   
то техните компоненти от множеството атрибути $Y$ могат да бъдат разменени   
и така получените 2 нови кортежа отново ще бъдат принадлежат на релацията.   

С други думи: 

```def```  
Нека $\alpha, \beta$- множества от атрибути на релацията $R$  
...
$\hspace{52mm} t_{1}[R \setminus (\beta \cup \alpha)] = t_{4}[R \setminus (\beta \cup \alpha)],$\
$\hspace{52mm} t_{2}[R \setminus (\beta \cup \alpha)] = t_{3}[R \setminus (\beta \cup \alpha)]]$  


Разбира се, тази дефиниция е предопределена.  
Необходим само единия коретеж.  
Другият се подразбира, поради кванторите за всеобщност
